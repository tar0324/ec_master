		...
		<div class="inquiry">
			<div class="content_header">
				...
				<p style="font-size: 12px;">
					회원정보에 저장된 성명과 휴대전화번호를 이용하여 아이디를 찾을 수 있습니다.</p>
					<c:if test="${check == 1}">
						<h1>일치하는 정보가 존재하지 않습니다.</h1>
					</c:if>
					<c:if test="${check == 0}">
						<h1>고객님의 아이디는 [${user_id }] 입니다.</h1>
					</c:if>
			</div>
			<div style="text-align: center" class="ok">
				<input type="button" id="find_pwd2" name="find_pwd2"
					onclick="location.href='${contextPath}/user/find_pwd.do'"
					value="비밀번호 찾기"> <input type="button" id="login2"
					name="login2"
					onclick="location.href='${contextPath}/user/loginForm.do'"
					value="로그인">
			</div>
		</div>
